...
export type filterElement = Element & {
  disabled: boolean
  hidden: boolean
  type: string
  tabIndex: number
...
export const getActiveElement = (element: HTMLElement) => {
  const nodes: HTMLElement[] = [];
  const acceptNode = (node: filterElement) => {
  ...
  const walker = document.createTreeWalker(element, NodeFilter.SHOW_ELEMENT, { acceptNode })
...
export const getEdges = (container: HTMLElement) => {
  const focusable = getActiveElement(container)
